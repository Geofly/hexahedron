...
public:
	...
	struct position {
		...
		void addInsSim(idx ref_ind, const char * ins, idx cnt) {
			static sStr bbuf;
			bbuf.cut0cut();
			bbuf.printf("%" DEC "-ins-%s", ref_ind, ins);
			bbuf.add0();
			if (!referenceSimil(bbuf.ptr()))
				referenceSimil[bbuf.ptr()] = cnt;
			else
		...
		inline idx getACGTCoverage(bool withAmbiguities) //ACGT and read supported Deletion
				{
			idx res = getBaseCoverage(baseA) + getBaseCoverage(baseC)
					+ getBaseCoverage(baseG) + getBaseCoverage(baseT);
			if (withAmbiguities)
				res += getBaseCoverage(baseN);
			if (res < 0)
				res = 0;
			return res;
		}
		inline idx getCoverage(bool withAmbiguities) //ACGT and read supported Deletion
				{
			idx res = coverage;
			if (!withAmbiguities)
				res -= base[baseN];
		...
		inline bool isSupportedInsertion(idx * compare, real cutoff, idx minCov,
				idx * cov = 0) {
			return isCoverageSupported(cov ? *cov : base[baseIns], compare,
					cutoff, minCov);
		}

		inline bool isInsertionPhased(idx call, idx * compare, real cutoff,
				idx minCov, idx * cov = 0, bool isConfirmed = false) {
			idx cur_cov = (cov ? *cov : base[baseIns]), closest_call, min_dist =
					sIdxMax, cur_dist = 0;
			...
			if (closest_call != call)
				return false;
			if (!isConfirmed)
				return !isCoverageBifurcating(sAbs(base[call] - cur_cov),
						compare, cutoff, minCov);
			return (((real) cur_cov) / coverage) >= cutoff;
		}

		inline bool isCoverageBifurcating(idx cov, idx * compare, real cutoff,
				idx minCov) {
		...
		inline bool isCoverageSupported(idx cov, idx * compare, real cutoff,
				idx minCov) {
		...
		inline real getLikeability(void) {
			if (getCoverage(false) == 1)
				return 0;
			return (real) ((getBaseCoverage(baseA) * getBaseCoverage(baseC))
					+ (getBaseCoverage(baseA) * getBaseCoverage(baseG))
					+ (getBaseCoverage(baseA) * getBaseCoverage(baseT))
					+ (getBaseCoverage(baseA) * getBaseCoverage(baseDel))
					+ (getBaseCoverage(baseA) * getBaseCoverage(baseGap))
					+ (getBaseCoverage(baseC) * getBaseCoverage(baseG))
					+ (getBaseCoverage(baseC) * getBaseCoverage(baseT))
					+ (getBaseCoverage(baseC) * getBaseCoverage(baseDel))
					+ (getBaseCoverage(baseC) * getBaseCoverage(baseGap))
					+ (getBaseCoverage(baseG) * getBaseCoverage(baseT))
					+ (getBaseCoverage(baseG) * getBaseCoverage(baseDel))
					+ (getBaseCoverage(baseG) * getBaseCoverage(baseGap))
					+ (getBaseCoverage(baseT) * getBaseCoverage(baseDel))
					+ (getBaseCoverage(baseG) * getBaseCoverage(baseGap))
					+ (getBaseCoverage(baseDel) * getBaseCoverage(baseGap)))
					/ (getCoverage(false) * (getCoverage(false) - 1));
		}
		inline idx setbasecall() {
			idx max = 0;
			baseCalls maxI = baseNone;
			for (idx i = baseFirst; i <= baseLast; ++i) {
				if (i == baseIns || i == baseN)
					continue;
				if (base[i] > max) {
					max = base[i];
					maxI = (baseCalls) i;
		...
		inline bool isGap(real thrshld) {
			return (((real) getBaseCoverage(baseGap) / getCoverage(true))
					>= thrshld);
		}
		inline bool isN() {
		...
		inline bool isDiff(idx alBase, real l_cutoff) {
			if (alBase == baseN || isN() || (basecall == alBase))
				return false;
			return getBaseConfidence((sBioseqpopul::baseCalls) alBase)
					< l_cutoff;
		}
		...
		bool isBifurcatingPosition(real cutoff, idx minCov, idx * firstInd = 0,
				idx * secInd = 0, idx * compare = 0) {
	...
	struct clonePosition {
		...
		inline void setCoverage(idx val) {
			if (val < 0)
				val = 0;
			baseCov = (baseCov & 0xF000000000000000)
					| (val & 0xFFFFFFFFFFFFFFF);
		}
	...
	struct cloneSummary {
		idx start, end /*non inclusive*/, first_bif_pos, parentClID, clID,
				mergeclID;
	...
	struct clonePat {
		...
		idx current() {
			return (pattern.dim() > 0) ?
					(curpos < pattern.dim() ? pattern[curpos].position : 0) : -1;
		}
		idx last() {
			return (pattern.dim() > 0) ?
					pattern[pattern.dim() - 1].position : -1;
		}
	...
	static idx sortClonesComparator(void * parameters, void * arr, idx i1,
			idx i2);
	...
	static const char * convertBaseCallVector2String(sVec<idx> & basecalls,
			sStr & buf) {
		return convertBaseCallArr2String(basecalls.ptr(), basecalls.dim(), buf);
	}
	static const char * convertBaseCallArr2String(idx * basecalls, idx basecnt,
			sStr & buf) {
		buf.cut0cut();
	...
	static idx convertString2BaseCallVector(const char * basecallstr,
			sVec<idx> & basecalls, idx slen = 0) {
	...
	static bool isDiffBases(idx * bases1, idx bcnt1, idx * bases2, idx bcnt2) {
		...
		for (; i < bcnt1 && (bases1[i] == bases2[i]); ++i) {
			if (bases1[i] == baseN || bases2[i] == baseN)
				return false;
		}
		return i != bcnt1;
	}
...
public:
	...
	//  base holds the base in this clone that created the bifurc from the father clone
	inline idx length() {
		return (_vec.dim() / 2);
	}
	idx curMatPOS;
	...
	idx expMatPOS(idx matPos) {
		return matPos >= curMatPOS ?
				matPos - curMatPOS : (dim() - curMatPOS + matPos);
	}
...
public:
	...
	bool wasBifurcationConsidered(idx new_pos, sVec<idx> &new_bases) {
		for (idx i = 0; i < prevBif.dim(); ++i) {
			...
			if (cur_bif->position == new_pos
					&& !sBioseqpopul::isDiffBases(cur_bif->bases.ptr(),
							cur_bif->bases.dim(), new_bases.ptr(),
							new_bases.dim())) {
				return true;
			}
		}
	...
	bool valid() {
		return !(killed || dead || merged
				|| (length() + curSubPOS < startSubPOS));
	}
	...
	inline idx getfirstNonZero(idx * start = 0, idx * end = 0) {
		idx st = start ? *start : getrawfirst(), ed =
				end ? *end : getTailLast();
	...
	inline void addL(idx isx, idx rpts, sBioseqpopul::baseCalls letter,
			idx ref_sim_ind) {
		pos(isx)->add(rpts, letter, _report_ref ? ref_sim_ind : 0);
	}
	inline void removeL(idx isx, idx rpts, sBioseqpopul::baseCalls letter,
			idx ref_sim_ind) {
...
public:
	...
	sCloneConsensus * ptr(idx i = 0) {
		return _vec.ptr(i);
	}
	;
	sCloneConsensus * add(idx i = 0) {
		return _vec.add();
	}
	;
	sCloneConsensus * ptrx(idx i = 0) {
		_frame_shifts.resize(i + 1);
		return _vec.ptrx(i);
	}
	;
	...
	idx getClonesPositionsOnLocation(idx pos, sVec<idx> &cloneIDs,
	...
	const char * contructSimilaritiesID(idx icl, idx pos,
			const char * reference);

	static idx sortPositionsComparator(void * parameters, void * arr, idx i1,
			idx i2);
	...
	idx frame_pos(idx icl, idx pos) {
		...
		if (cr_frm_shifts->dim() == 0)
			return sub_start + pos;
		idx i = 0, tot_ins = 0;
	...
	bool mergePositions(sCloneConsensus * dstCl, sCloneConsensus * srcCl,
			idx frmStart = 0, idx frmEnd = 0, bool adjustForGaps = false);

	idx getSimilarities(idx cloneID, idx framePos, sBioseqpopul::position * p);
	idx getInsSimilarities(idx cloneID, idx framePos,
			sBioseqpopul::position * p, const char * ins);
		...
		if (oldCl && newCl && oldCl != newCl) {
			for (idx j = 0; j < dim(); ++j) {
				...
				if (j > oldCl->summary.clID) {
					if (chCl->summary.hasParent()
							&& chCl->summary.parentClID == oldCl->summary.clID)
						chCl->summary.parentClID = newCl->summary.clID;
					if (chCl->summary.hasMerged()
							&& chCl->summary.mergeclID == oldCl->summary.clID)
	...
	void fixParentDependencies(sCloneConsensus * cl) {
		if (cl) {
			for (idx j = 0; j < dim(); ++j) {
				sCloneConsensus * chCl = ptr(j);
				if (j > cl->summary.clID && chCl->summary.hasParent()
						&& chCl->summary.parentClID == cl->summary.clID
						&& chCl->summary.first_bif_pos < cl->summary.start) {
					sCloneConsensus * pCl = getParentOnLocation(
							cl->summary.clID, chCl->summary.first_bif_pos);
					chCl->summary.parentClID =
							pCl ? pCl->summary.clID : chCl->summary.clID;
				}
				if (chCl->summary.hasMerged()
						&& chCl->summary.mergeclID == cl->summary.clID
						&& chCl->getSummaryEnd() < cl->summary.start) {
					sCloneConsensus * pCl = getParentOnLocation(
							cl->summary.clID, chCl->getSummaryEnd());
					chCl->summary.mergeclID =
							pCl ? pCl->summary.clID : chCl->summary.clID;
				}
			}
		}
	...
	void fixMergedDependencies(sCloneConsensus * cl) {
		if (cl) {
			for (idx j = 0; j < dim(); ++j) {
				...
				if (j > cl->summary.clID && chCl->summary.hasParent()
						&& chCl->summary.parentClID == cl->summary.clID
						&& chCl->summary.first_bif_pos > cl->getSummaryEnd()) {
					sCloneConsensus * mCl = getMergedOnLocation(
							cl->summary.clID, chCl->summary.first_bif_pos);
					chCl->summary.parentClID =
							mCl ? mCl->summary.clID : chCl->summary.clID;
				}
				if (chCl->summary.hasMerged()
						&& chCl->summary.mergeclID == cl->summary.clID
						&& chCl->getSummaryEnd() > cl->getSummaryEnd()) {
					sCloneConsensus * mCl = getMergedOnLocation(
							cl->summary.clID, chCl->getSummaryEnd());
					chCl->summary.mergeclID =
							mCl ? mCl->summary.clID : chCl->summary.clID;
	...
	bool areDifferent(sCloneConsensus * cl1, sCloneConsensus * cl2, idx start =
			0, idx end = 0);

	idx cleanOutClones(sVec<idx> &cloneIndex);
	idx sOutSimilarities(const char * flnm);
	idx createFrameMaps(const char * skp2gps, const char * gps2skp,
			const char * skp2gpsSpprt, const char * gps2skpSpprt);
};
...
public:
	...
	void addInsertion(idx cloneID, idx subpos, sVec<idx> & basecalls, idx rpts,
			idx subID = -1) {
		sDic<idx> & _insertions = setInsertionsDic(cloneID, subpos);
		static sStr cr_buf;
		cr_buf.cut0cut(0);
		const char * insertion_call =
				sBioseqpopul::convertBaseCallVector2String(basecalls, cr_buf);
	...
	void removeInsertion(idx cloneID, idx subpos, sVec<idx> & basecalls,
			idx rpts, idx subID = -1) {
		addInsertion(cloneID, subpos, basecalls, -rpts, subID);
	}

	idx getPhasedInsertionStr(sClone * w, idx ip, real cutoff, idx minBifCov,
			sStr &s_insertions, bool isConfirmed = false) {
		sBioseqpopul::position * p = w->pos(ip);
		if (p->isSupportedInsertion(0, cutoff, minBifCov)
				&& p->isInsertionPhased(p->basecall,
						_coverage.pos(ip - w->curSubPOS), cutoff, minBifCov, 0,
						true)) {
			idx ins_cov = 0, slen;
			const char * cr_ins = getDominantInsertionChar(w->clID, ip, slen,
					ins_cov);
			if (!cr_ins
					|| !p->isSupportedInsertion(0, cutoff, minBifCov, &ins_cov)
					|| !p->isInsertionPhased(p->basecall,
							_coverage.pos(ip - w->curSubPOS), cutoff, minBifCov,
							&ins_cov, isConfirmed))
				return 0;
	...
	bool getPhasedInsertionVec(sClone * w, idx ip, real cutoff, idx minBifCov,
			sVec<idx> &insertions, bool isConfirmed = false) {
		static sStr c_buf;
		c_buf.cut0cut(0);
		idx ins_cov = getPhasedInsertionStr(w, ip, cutoff, minBifCov, c_buf,
				isConfirmed);
		if (!ins_cov)
			return 0;
		sBioseqpopul::convertString2BaseCallVector(c_buf, insertions,
				c_buf.length());
		return ins_cov;
	}

	idx getPositionInsertionBasecalls(sClone * cl, idx ip, real cutoff,
			idx minBifCov, sVec<idx> & basecalls, idx * ins_cov = 0) {
		sBioseqpopul::position * p = cl->pos(ip);
		basecalls.cut(0);

		basecalls.vadd(1, p->basecall);
		idx l_ins_cov = getPhasedInsertionVec(cl, ip, cutoff, minBifCov,
				basecalls);
		if (ins_cov)
			*ins_cov = l_ins_cov;
		return basecalls.dim();
	...
	bool isDiffPositionInsertionBasecalls(sClone * cl1, sClone * cl2, idx ip,
			real cutoff, idx minBifCov) {
		...
		if (p1->isN() || p2->isN())
			return false;
		return sBioseqpopul::isDiffBases(basecalls1.ptr(), basecalls1.dim(),
				basecalls2.ptr(), basecalls2.dim());
	}

	bool isDiffPositionInsertion(sClone * cl1, sClone * cl2, idx ip,
			real cutoff, idx minBifCov) {
		...
		if (!p->isN()
				&& areBasecallsThere(cl2, ip, basecalls, cutoff, minBifCov))
			return false;
		basecalls.cut(0);
		p = cl2->pos(ip);
		getPositionInsertionBasecalls(cl2, ip, cutoff, minBifCov, basecalls);
		if (!p->isN()
				&& areBasecallsThere(cl1, ip, basecalls, cutoff, minBifCov))
			return false;
	...
	bool areBasecallsThere(sClone *cl, idx ip, sVec<idx> &basecalls,
			real cutoff, idx minBifCov) {
		sBioseqpopul::position * p = cl->pos(ip);
		if (!basecalls.dim()) {
		...
		if (((real) p->getBaseCoverage(
				static_cast<sBioseqpopul::baseCalls>(c_base))
				/ p->getCoverage(false)) < cutoff)
			return false;
		if (basecalls.dim() > 1) {
			idx ins_cov = isInsertionInPos(cl->clID, ip, basecalls.ptr(1),
					basecalls.dim() - 1);
	...
	idx getDominantInsertion(idx cloneID, idx subpos, sVec<idx> & basecalls,
			idx &cnt) {
		idx slen = 0;
		const char * cr_ins = getDominantInsertionChar(cloneID, subpos, slen,
				cnt);
		if (!cr_ins)
			return 0;
	...
	const char * getDominantInsertionChar(idx cloneID, idx subpos, idx &slen,
			idx &cnt) {
		sDic<idx> * _insertions = getInsertionsDic(cloneID, subpos);
		if (!_insertions)
			return 0;
	...
	idx isInsertionInPos(idx cloneID, idx subpos, idx * basecalls,
			idx basecnt) {
		static sStr cr_buf;
		sDic<idx> * _insertions = getInsertionsDic(cloneID, subpos);
		if (!_insertions)
			return 0;
		cr_buf.cut0cut();
		const char * insertion_call = sBioseqpopul::convertBaseCallArr2String(
				basecalls, basecnt, cr_buf);
		idx * in_rpts = _insertions->get(insertion_call);
		if (!in_rpts)
			return 0;
		return *in_rpts;
	...
	idx getSecondDominantInsertion(idx cloneID, idx subpos,
			sVec<idx> &basecalls, idx &cnt) {
		sDic<idx> * _insertions = getInsertionsDic(cloneID, subpos);
		if (!_insertions)
			return 0;
		sStr id_buf;
		...
		sBioseqpopul::convertString2BaseCallVector(
				(const char *) _insertions->id(sec_ind, &slen), basecalls,
				slen);
		return basecalls.dim();
	}
	...
	void reorder(idx * rInds, idx start, idx cnt) {
		...
		for (idx i = 0; i < start + cnt; ++i) {
			jcl = ptr(i);
			if (jcl->mergedToCl < 0)
				jcl->mergedToCl = -jcl->mergedToCl - 1;
			if (jcl->parentClID < 0)
				jcl->parentClID = -jcl->parentClID - 1;
		}
		for (idx i = 0; i < cnt; ++i) {
	...
	inline bool isFuzzy(sClone * w, idx p, real cutoff) {
		return isFramePositionFuzzy(p, cutoff)
				|| isClonalPositionFuzzy(w, p, cutoff);
	}
	inline bool isFramePositionFuzzy(idx p, real cutoff) {
		if (!avCoverage())
			return true;
		return (real) *_coverage.pos(p - getMainClone()->curSubPOS)
				/ avCoverage() < cutoff;
	}
	...
	inline bool isClonalPositionFuzzy(sClone * w, idx p, real cutoff) {
		if (*_coverage.pos(p - getMainClone()->curSubPOS) <= 0)
			return true;
		return ((real) w->pos(p)->getCoverage(false)
				/ sMax((real) *_coverage.pos(p - getMainClone()->curSubPOS),
						w->avCoverage()) < cutoff);
	}
	inline bool isClonalInsertionFuzzy(sClone * w, idx p, real cutoff,
			idx &ins_ind) {
		if (*_coverage.pos(p - getMainClone()->curSubPOS) <= 0)
			return true;
		idx comb_cov = sMax(
				(real) *_coverage.pos(p - getMainClone()->curSubPOS),
				w->avCoverage());
		if (((real) w->pos(p)->getBaseCoverage(sBioseqpopul::baseIns) / comb_cov
				< cutoff))
			return true;
		idx ins_cnt;
	...
	idx getSingleFingPrint(sBioseqpopul::clonePat * p, idx parentClID,
			idx childID, real fuzzy_coverages_threshold, idx bif_start,
			idx * start = 0, idx * end = 0);
		...
		for (idx i = start; i < ed; ++i) {
			if (src->pos(i)->base[sBioseqpopul::baseIns]) {
				...
				for (idx in = 0; in < _insertions->dim(); ++in) {
					...
					sBioseqpopul::convertString2BaseCallVector(basecalls,
							ins_vec, slen);
	...
	static idx getAlBoundaries(sBioal * bioal, idx * alList, idx posStart,
			idx posEnd, idx &alStart, idx &alEnd, idx alCnt);
