...
private:
	...
	sVioDB vioDB;
	enum eVioPopTypes {
		eTypeClone = 1, eTypeSeqCov, eTypeStats, eTypeSummary, eTypeSortArr
	};
	enum eCloneRlts {
...
public:
	...
	struct cloneProfPos {
		...
		idx sum() {
			return pos[sBioseqpopul::baseA] + pos[sBioseqpopul::baseC]
					+ pos[sBioseqpopul::baseG] + pos[sBioseqpopul::baseT]
					+ pos[sBioseqpopul::baseDel] + pos[sBioseqpopul::baseN];
		}
		cloneProfPos() {
			sSet(this, 0);
		}
	...
	struct seqCovPosition {
		...
		inline char baseChar() {
			return isAnyGap() ?
					'-' : (isN() ? 'N' : sBioseq::mapRevATGC[base()]);
		}
		inline bool isN() {
			return base() == sBioseqpopul::baseN;
		}
		inline bool isMultiGap() {
			return base() == sBioseqpopul::baseGap;
		}
		inline bool isSupportedGap() {
			return base() == sBioseqpopul::baseDel;
		}
		bool isCoverageGap() {
			return coverage() == 0;
		}
		inline bool isAnyGap() {
			return isSupportedGap() || isMultiGap() || isCoverageGap();
		}
		idx coverage() {
			return (baseCov & 0xFFFFFFFFFFFFFFF);
		}
		void setCoverage(idx val) {
			if (val < 0)
				val = 0;
			baseCov = (baseCov & 0xF000000000000000)
					| (val & 0xFFFFFFFFFFFFFFF);
		}
		void setBase(idx val) {
			if (val < 0)
				val = sBioseqpopul::baseGap;
	...
	sViopop(const char * InputFilename = 0, const char * similFlnm = 0,
			const char * gap2skiFlnm = 0, const char * skip2gapFlnm = 0,
			idx mode = sMex::fReadonly) {
	...
	sViopop * init(const char * InputFilename = 0, const char * similFlnm = 0,
			const char * gap2skiFlnm = 0, const char * skip2gapFlnm = 0,
			idx mode = sMex::fReadonly) {
	...
	sBioseqpopul::cloneSummary * getCl(idx iAlIndex, idx * sortArr = 0) {
		return (sBioseqpopul::cloneSummary *) vioDB.Getbody(eTypeClone,
				iAlIndex + 1, 0);
	}
	void getCov(idx iAlIndex, sVec<idx> & cov, idx * sortArr = 0) {
		seqCovPosition * seqcov = 0;
		idx * pMatch = (idx*) vioDB.GetRelationPtr(eTypeClone,
				(idx) iAlIndex + 1, eClone2SeqCov, 0, 0), bodysize = 0;
		if (pMatch)
			seqcov = (seqCovPosition *) vioDB.Getbody(eTypeSeqCov, *pMatch,
					&bodysize);
	...
	seqCovPosition * getSeqCov(idx iAlIndex, idx * bodysize,
			idx * sortArr = 0) {
		seqCovPosition * seqcov = 0;
		idx * pMatch = (idx*) vioDB.GetRelationPtr(eTypeClone,
				(idx) iAlIndex + 1, eClone2SeqCov, 0, 0);
		if (pMatch)
			seqcov = (seqCovPosition *) vioDB.Getbody(eTypeSeqCov, *pMatch,
					bodysize);
		if (bodysize) {
	...
	idx getPositionSimil(idx cl, idx pos, idx refCnt, real * res,
			bool percentage = false, sVec<contigComp> * comp = 0) {
		if (!simil) {
			return 0;
		}
		idx tot = 0;
	...
	real getMergedPosSimils(idx clID, idx pos, idx iS, idx cov,
		...
		if (contigs) {
			for (idx j = 0; j < contigs->dim(); ++j) {
				res += (real) getPositionSingleSimil(*(contigs->ptr(j)), pos,
						iS);
			}
			res = res / cov;
		} else {
	...
	bool getSimilAboveThrshld(sVec<real> &simil, sStr & res, real thrshld,
			sBioseq * Sub = 0) {
		res.cut(0);
		bool ret = false;
	...
	idx getMergedPositionSingleSimil(idx cl, idx pos, idx ref,
	...
	idx getGap2SkipContigPos(sBioseqpopul::cloneSummary *cl, idx pos,
			sVec<idx> * contigMap = 0, bool forceValid = false) {
		idx res = 0;
	...
	idx getSkip2GapContigPos(sBioseqpopul::cloneSummary *cl, idx pos,
			sVec<idx> * contigMap = 0) {
		if (contigMap && contigMap->dim()) {
	...
	idx getFramePos(sBioseqpopul::cloneSummary * cl, idx pos,
			sVec<idx> * contigMap = 0, bool forceValid = false) {
		if ((!gap2skipMap || cl->clID > gap2skipMap->dim())
	...
	inline idx getContigPos(sBioseqpopul::cloneSummary * cl, idx pos,
			sVec<idx> * contigMap = 0, bool forceValid = false) {
		return getContigDistance(cl, pos, 0, contigMap, forceValid);
	}

	idx getGap2SkipFramePos(sBioseqpopul::cloneSummary * cl, idx pos,
			sVec<idx> * contigMap = 0, bool forceValid = false);

	idx getContigDistance(sBioseqpopul::cloneSummary * cl, idx end, idx start =
			0, sVec<idx> * contigMap = 0, bool forceValid = false);

	inline bool isMultiGap(sBioseqpopul::cloneSummary * cl, idx pos,
			sVec<idx> * contigMap = 0) {
		if (contigMap && contigMap->dim()) {
...
//                }
	...
	inline bool isSupportedGap(sBioseqpopul::cloneSummary * cl, idx pos,
			sVec<idx> * contigMap = 0) {
		if (contigMap && contigMap->dim()) {
...
//                }
	...
	inline bool isAnyGap(sBioseqpopul::cloneSummary * cl, idx pos,
			sVec<idx> * contigMap = 0) {
		if (contigMap && contigMap->dim()) {
			return *contigMap->ptr(pos) == -2;
		} else {
	...
	sBioseqpopul::cloneStats * getStats(idx iAlIndex, idx * sortArr = 0) {
		idx * pMatch = (idx*) vioDB.GetRelationPtr(eTypeClone,
				(idx) iAlIndex + 1, eClone2Stats, 0, 0);
		return pMatch ?
				(sBioseqpopul::cloneStats*) vioDB.Getbody(eTypeStats, *pMatch,
						0) :
				(sBioseqpopul::cloneStats*) 0;
	}
	idx * getArrSort() {
	...
	sBioseqpopul::cloneSummary * getFatherCl(idx iAlIndex, idx * sortArr = 0) {
		idx * pMatch = (idx*) vioDB.GetRelationPtr(eTypeClone,
				(idx) iAlIndex + 1, eClone2FatherClone, 0, 0);
		return pMatch ?
				(sBioseqpopul::cloneSummary*) vioDB.Getbody(eTypeClone, *pMatch,
						0) :
				(sBioseqpopul::cloneSummary*) 0;
	}
	sBioseqpopul::cloneSummary * getMergedCl(idx iAlIndex, idx * sortArr = 0) {
		idx * pMatch = (idx*) vioDB.GetRelationPtr(eTypeClone,
				(idx) iAlIndex + 1, eClone2MergeClone, 0, 0);
		return pMatch ?
				(sBioseqpopul::cloneSummary*) vioDB.Getbody(eTypeClone, *pMatch,
						0) :
				(sBioseqpopul::cloneSummary*) 0;
	...
	idx * listBifCl(idx iAlIndex, idx * relCount, idx * sortArr = 0) {
		...
		return (idx*) vioDB.GetRelationPtr(eTypeClone, iAlIndex + 1, eTypeClone,
				(idx*) relCount, 0);
	}

	idx Digest(const char* outputfilename, sFrameConsensus * rawPopulation,
...
public:
	...
	struct ParamCloneIterator {
		idx flags, iCln, step, resolution, minCov, minLen, minSup, *normCov,
				mergeHidden, showSimil, sStart, sEnd, wrap, breaksminLen,
				covThrs, mc_iters;
//            const char * hideClones;
	...
	typedef idx (*typeCallbackIteratorFunction)(sViopop * viopop,
			ParamCloneIterator * param, idx cloneIndex);

	idx iterateClones(idx * iVis, idx start, idx cnt,
			typeCallbackIteratorFunction callbackFunc,
			ParamCloneIterator * params, idx * arrSortClon = 0);

	idx printAllCoverageClones(idx * iVis, idx start, idx cnt, sStr & out,
			idx * arrSortClon = 0);
	idx printAllSequenceClones(idx * iVis, idx start, idx cnt, sStr & out,
			idx * arrSortClon = 0);

	idx getPermutationsCompositions(const char * clones,
			sVec<sVec<sViopop::cloneRegion> > & out, sVec<idx> & clIds,
			ParamCloneIterator * params = 0);

	idx getPredictedGlobal(sVec<sVec<sViopop::cloneRegion> > & out,
			sVec<real> & clIds, ParamCloneIterator * params = 0);

	idx getExtendedCompositions(const char * clones,
			sVec<sVec<sViopop::cloneRegion> > & out, sVec<idx> & clIds,
			ParamCloneIterator * params = 0);
	idx getExtendedComposition(sBioseqpopul::cloneSummary * cl,
			sVec<sViopop::cloneRegion> * c_Out,
			ParamCloneIterator * params = 0);
	idx getContigCompositions(const char * clones_input,
			sVec<sVec<sViopop::cloneRegion> > & out, sVec<idx> & clIds,
			ParamCloneIterator * params = 0);
	idx getContigComposition(sBioseqpopul::cloneSummary * cl,
			sVec<sViopop::cloneRegion> * c_Out,
			ParamCloneIterator * params = 0);

	const char * printTitle(sStr & title, ParamCloneIterator * param,
			idx contgId, idx start = 0, idx end = 0, const char * sep = ",");
	void printTitleFreq(sStr & title, ParamCloneIterator * param, idx contgId) {
		if (param && param->frequencies)
			title.printf(" Freq=%lf", param->frequencies[contgId]);
	}
	;

	idx printContigsPrevalence(sVec<sVec<sViopop::cloneRegion> > & composition,
			sVec<idx> & clIds, sStr & out, ParamCloneIterator * params);

	idx printContig(sVec<sVec<sViopop::cloneRegion> > & compositions,
			sVec<idx> & clIds, sStr & out, ParamCloneIterator * params,
			idx print_type);
	idx printContigSequences(sVec<sViopop::cloneRegion> & composition,
			sStr & out, ParamCloneIterator * params, idx curclID,
			idx offset = 0);
	idx printContigAlignments(sVec<sViopop::cloneRegion> & composition,
			sStr & out, ParamCloneIterator * param, idx curclID,
			idx offset = 0);
	idx printContigCoverages(sVec<sViopop::cloneRegion> & composition,
			sStr & out, ParamCloneIterator * param, idx curclID,
			idx offset = 0);
	idx printContigComposition(sVec<sViopop::cloneRegion> & composition,
			sStr & out, ParamCloneIterator * param, idx curclID,
			idx offset = 0);
	idx printContigBreakpoints(sVec<sViopop::cloneRegion> & composition,
			sStr & out, ParamCloneIterator * param, idx curclID,
			idx offset = 0);
	idx printContigSummary(sVec<sViopop::cloneRegion> & composition, sStr & out,
			ParamCloneIterator * param, idx offset = 0);
	...
	bool cloneOverlap(sBioseqpopul::cloneSummary * clA,
			sBioseqpopul::cloneSummary * clB);

	bool isCloneAncestor(sBioseqpopul::cloneSummary * t_sum,
			sDic<bool> & clones);
	bool isCloneAncestor(sBioseqpopul::cloneSummary * t_sum1,
			sBioseqpopul::cloneSummary * t_sum2);

	bool printSimilaritiesinJSON(sStr & out, idx cl, idx pos, idx cov,
			idx rfCnt, ParamCloneIterator * params,
			sVec<contigComp> * cComp = 0);

	void printPositionCoverageinJSON(sStr & out, idx pos, char let, idx cov,
			const char * sim, bool abs, idx t_cov);

	const char * contructSimilaritiesID(idx icl, idx pos, idx reference) {
		return _t_buf.printf(0, "%" DEC "-%" DEC "-%" DEC, icl, pos, reference);
	}

	bool buildMergeDictionary(ParamCloneIterator * params,
			sDic<sVec<idx> > & mergeTree, idx clCnt);
	idx buildMergeCompositionDictionary(ParamCloneIterator * params,
	...
	bool getValidCloneComposition(sBioseqpopul::cloneSummary * cl,
			sVec<contigComp> & c_comp);
	bool mergeClone(sBioseqpopul::cloneSummary * cl,
			sBioseqpopul::cloneSummary &resCl, sVec<contigComp> & comp,
			ParamCloneIterator * params = 0, sVec<seqCovPosition> * seqCov = 0,
			sVec<idx> * gap2skpMap = 0, sVec<idx> * skp2gapMap = 0);

	idx getMergedFramePos(sBioseqpopul::cloneSummary * cl, idx pos,
			ParamCloneIterator * params = 0, bool forceValid = false);
	idx getMergedContigPos(sBioseqpopul::cloneSummary * cl, idx pos,
			ParamCloneIterator * params = 0, bool forceValid = false);

	idx clonesCoverage(ParamCloneIterator * params,
			sBioseqpopul::cloneSummary * Sum, sVec<seqCovPosition> * seqCov,
			idx refCnt, sVec<contigComp> * cComp = 0,
			sVec<idx> * skp2gapMap = 0, sVec<idx> * gap2skpMap = 0);
	idx clonesDifferences(ParamCloneIterator * params,
			sBioseqpopul::cloneSummary * parentCl,
			sBioseqpopul::cloneSummary * childCl,
			sVec<seqCovPosition> * pSC = 0, sVec<seqCovPosition> * cSC = 0,
			sVec<idx> * child_gap2skpMap = 0,
			sBioseqpopul::cloneStats * c_stats = 0);

	static idx printAllClones(sViopop * viopop, ParamCloneIterator * params,
			idx clIndex);
	static idx printHierarchySingle(sViopop * viopop,
			ParamCloneIterator * params, idx clIndex);
	static idx printCoverageSingle(sViopop * viopop,
			ParamCloneIterator * params, idx clIndex);
	static idx getClonesOnPosition(sViopop * viopop, idx pos,
			sVec<idx> * clInds);
...
public:
	...
	idx findMerged(idx end, sBioseqpopul::cloneSummary * clone,
			sVec<idx> & res) {
		node * nd = 0;
		for (idx i = 0; i < _nodes.dim(); ++i) {
			nd = _nodes.ptr(i);
			if (nd->clone.hasMerged() && nd->clone.end == end && nd->end == end
					&& nd->clone.mergeclID == clone->clID) {
				res.vadd(1, i);
			}
		}
		return res.dim();
	...
	node * addNode(idx start, idx end, idx irng,
			sBioseqpopul::cloneSummary & cls) {
		node * n = _nodes.add();
		n->start = start;
		n->end = end;
		n->irng_s = irng;
	...
	node * getPrevNode(node * n, real freq) {
		if (!n->previous.dim())
			return 0;
		if (n->previous.dim() == 1)
			return _nodes.ptr(n->previous[0]);
	...
	idx traverse(idx seed_ind, sVec<sViopop::cloneRegion> & out,
			bool min_diversity);
