...
void sFrame::correctInsertions(sClone * w) {
	...
	for (idx ip = st; ip < end; ++ip) {
		...
		if (_insertions && _insertions->dim() > 1) {
			...
			const char * ins = getDominantInsertionChar(w->clID, ip, len, rpts),
					*c_ins = 0;
...
idx sFrame::getSingleFingPrint(sBioseqpopul::clonePat * p, idx parentClID,
		idx childID, real fuzzy_coverages_threshold, idx bif_start, idx * start,
		idx * end) {
	...
	idx st = start ? *start : w1->getrawfirst(), ed =
			end ? *end : w1->getTailLast();
	...
	for (; i < ed; ++i) {
		...
		if (isDiffPositionInsertionBasecalls(w0, w1, i, _cutoff, 0)) {
			if (!isClonalPositionFuzzy(w0, i, fuzzy_coverages_threshold)
					&& !isClonalPositionFuzzy(w1, i,
							fuzzy_coverages_threshold)) { //both coverage have to be above the fuzzy threshold
...
const char * sFrame::diff(idx cl1, idx cl2, bool onlydiffs /*= true*/) {
	sClone * w1 = ptr(cl1);
	sClone * w2 = ptr(cl2);
	if (!w1 || !w2) {
		return 0;
	...
	for (idx i = 0; i < w1->dim(); ++i) {
		...
		sStr flag;
		flag.printf("   ");
		if (p1->basecall != p2->basecall && p1->basecall >= 0
				&& p2->basecall >= 0) {
			isdiff = true;
			flag.printf(0, "***");
			++cnt;
		...
		out->printf(
				"|%3" DEC "|%7" DEC "|   %c   |%7" DEC "|   %c   |%7" DEC "|%7s|\n",
				i, w1->subPos(i), p1->basechar(), cov_base1, p2->basechar(),
				cov_base2, flag.ptr());
	}
...
idx sFrame::getAlBoundaries(sBioal * bioal, idx * alList, idx posStart,
		idx posEnd, idx &iAlStart, idx &iAlEnd,
		idx alCnt/*,sVec<sBioseqAlignment::Al *> * alSub,sVec< sVec<idx> > * alSubLUT*/) {
	sBioseqAlignment::Al * hdrA = bioal->getAl(
			alList ? alList[iAlStart] : iAlStart);
...
const char * sClone::print(real cutoff, idx minCov, sStr * out) {
	...
	out->printf(
			"|Clone__%7" DEC "_____________________________________________________________________________|\n",
			clID);
	out->printf(
			"| # |  sub  |   A   |   C   |   G   |   T   |  Del  |  Ins  |  GAP  |  Any  |  SNV  | flags |\n");
	out->printf(
			"|---+-------+-------+-------+-------+-------+-------+-------+-------+-------+-------+-------|\n");
	sBioseqpopul::position * p = 0;
	for (idx i = 0; i < dim(); ++i) {
		...
		out->printf(
				"|%3" DEC "|%7" DEC "|%7" DEC "|%7" DEC "|%7" DEC "|%7" DEC "|%7" DEC "|%7" DEC "|%7" DEC "|%7" DEC "|%7s|%7s|\n",
				i, subPos(i), p->base[sBioseqpopul::baseA],
				p->base[sBioseqpopul::baseC], p->base[sBioseqpopul::baseG],
				p->base[sBioseqpopul::baseT], p->base[sBioseqpopul::baseDel],
				p->base[sBioseqpopul::baseIns], p->base[sBioseqpopul::baseGap],
				p->base[sBioseqpopul::baseN],
				p->isBifurcatingPosition(cutoff, minCov) ? "  ***  " : "",
				flag.ptr());
	}
	out->printf(
			"|-------------------------------------------------------------------------------------------|\n");
...
const char * sFrame::printCoverage(sStr * out) {
	...
	for (idx i = 0; i < _coverage.dim(); ++i) {
		...
		out->printf("|%3" DEC "|%7" DEC "|%7" DEC "|%7s|\n", i,
				getMainClone()->subPos(i), _coverage[i], flag.ptr());
	}
	out->printf("|---------------------------|\n");
	return out->ptr();
}

idx sFrame::getFirstDiff(sClone * w1, sClone * w2, idx start, idx end,
		real cutoff) {
	for (idx i = start; i < end; ++i) {
		if (isDiffPositionInsertionBasecalls(w1, w2, i, cutoff, minBifCov)
				&& !this->isClonalPositionFuzzy(w1, i, cutoff)
				&& !this->isClonalPositionFuzzy(w2, i, cutoff))
			return i;
	}
	return -1;
}

idx sFrame::getLastDiff(sClone * w1, sClone * w2, idx start, idx end,
		real cutoff) {
...
idx sBioseqpopul::sortClonesComparator(void * parameters, void * varr, idx i1,
		idx i2) {
...
idx sFrameConsensus::getClonesPositionsOnLocation(idx pos, sVec<idx> &clIDs,
...
idx sFrameConsensus::getSimilarities(idx cloneID, idx framePos,
		sBioseqpopul::position * p) {
	idx simils = 0, slen = 0;
	idx tot_sim = 0;
	const char * cr_id = 0;
	sStr str_id;
	...
	for (idx i = 0; i < p->referenceSimil.dim(); ++i) {
		...
		if (!strstr(str_id.ptr(), inssep) && p->referenceSimil[i] > 0) {
			...
			c_p_sim = similarities.get(_simil_id_buf.ptr(),
					_simil_id_buf.length());
			if (c_p_sim) {
				*c_p_sim += p->referenceSimil[i];
			} else {
				*similarities.set(_simil_id_buf.ptr(), _simil_id_buf.length()) =
						p->referenceSimil[i];
			}
...
idx sFrameConsensus::getInsSimilarities(idx cloneID, idx framePos,
...
sCloneConsensus * sFrameConsensus::getParentOnLocation(idx iCl, idx pos) {
	...
	if (sOverlap(pos, pos, cl->summary.start, cl->getSummaryEnd() - 1)) { // -1 because end is not inclusive
		return cl;
...
idx sFrameConsensus::getFirstMutation(idx iCl, idx pos /*= 0*/,
	...
	for (idx i = pos, i_p = pos;
			i < cl->seqCov.dim() && i_p < parent->seqCov.dim(); ++i, ++i_p) {
		...
		p2 = parent->getSubPos(i + cl_mut_start);
		if (!p2) {
			continue;
		}
		if (ignoreGaps) {
...
idx sFrameConsensus::getLastMutation(idx iCl, idx pos /*= 0*/,
		bool ignoreGaps /*= false*/) {
	sCloneConsensus * cl = ptr(iCl);
...
bool sFrameConsensus::mergePositions(sCloneConsensus * dstCl,
		sCloneConsensus * srcCl, idx start /*= 0*/, idx cnt /*= 0*/,
		bool adjustForGaps) {
	if (srcCl == dstCl)
		return false;
	if (!start) {
	...
	sStr ref_ind;
	if (sOverlap(dstCl->summary.start,
			dstCl->summary.start + dstCl->seqCov.dim() - 1, start,
			start + cnt - 1)) {
		...
		for (idx pos_src = start, pos_dst = start;
				pos_src < frmEnd && pos_dst < frmEnd; ++pos_src, ++pos_dst) {
			if (adjustForGaps) {
			...
			dstCl->seqCov.ptrx(pos_dst - dstCl->summary.start)->add(
					srcCl->seqCov.ptr(pos_src - srcCl->summary.start));
			for (idx i = 0; i < simil_size; ++i) {
				ref_ind.cut(0);
				ref_ind.addNum(i);
				idx * sv = similarities.get(
						contructSimilaritiesID(srcCl->summary.clID, pos_src,
								ref_ind));
				if (sv) {
					idx svv = *sv;
					idx * dv = similarities.get(
							contructSimilaritiesID(dstCl->summary.clID, pos_dst,
									ref_ind));
					if (dv) {
						*dv += svv;
					} else {
						dv = similarities.set(
								contructSimilaritiesID(dstCl->summary.clID,
										pos_dst, ref_ind));
						if (dv)
							*dv = svv;
						else {
	...
	for (idx i = 0; i < dim(); ++i) {
		...
		if (start_bif_dif > 1 && start_bif_dif < cl->seqCov.dim()) { //do not cut exactly on the mutation
			...
			if (cl->summary.hasParent()) {
				sCloneConsensus * pCl = ptr(cl->summary.parentClID), *onBpCl =
						pCl;

				for (p = cl->summary.start;
						p < start_bif_dif + cl->summary.start; ++p) {
		...
		if (cl->summary.hasMerged()) {
			...
			if (cl->summary.isLink() && last_bif_dif < cl->summary.start + 2) { //three position for dispaying purposes. One for bifurcation one for depth of coverage and one for merging
			...
			if (last_bif_dif < cl->summary.end) {
				cl->seqCov.del(1 + last_bif_dif - cl->summary.start,
						cl->getSummaryEnd() - last_bif_dif - 1);
...
//static
idx sFrameConsensus::sortPositionsComparator(void * parameters, void * varr,
		idx i1, idx i2) {
	sBioseqpopul::clonePosition * pA = ((sBioseqpopul::clonePosition *) varr)
			+ i1;
	sBioseqpopul::clonePosition * pB = ((sBioseqpopul::clonePosition *) varr)
			+ i2;
	return (pA->coverage() != pB->coverage()) ?
			((pA->coverage() > pB->coverage()) ? -1 : 1) : 0;
}
...
bool sFrameConsensus::reorderClones() {
	...
	for (idx i = sub_start; i <= sub_end; ++i) {
		...
		if (clIDs.dim() > 1) {
			sSort::sortSimpleCallback(sFrameConsensus::sortPositionsComparator,
					0, ret.dim(), ret.ptr(), sort_ind.ptr());
			if (sort_ind.dim()) {
...
idx sFrameConsensus::createFrameMaps(const char * skp2gps, const char * gps2skp,
...
const char * sFrameConsensus::contructSimilaritiesID(idx icl, idx pos,
		const char * reference) {
	_simil_id_buf.printf(0, "%" DEC "-%" DEC "-%s", icl, pos, reference);
	return _simil_id_buf;
}

idx sFrameConsensus::remapSimilarities(sDic<idx> *sim, idx icl, idx pos,
		idx remapped_icl) {
	...
	for (idx i = 0; i < simil_size; ++i) {
		...
		if (cov) {
			t_s.printf(0, "%s",
					contructSimilaritiesID(remapped_icl, pos, ref_ind));
...
void sFrameConsensus::remapSimilarities(sVec<idx> &vec_icl, sVec<idx> &vec_sort,
		idx pos) {
	...
	for (idx i = 0; i < vec_sort.dim(); ++i) {
		for (idx ir = 0; ir < simil_size; ++ir) {
			ref_ind.cut(0);
			ref_ind.addNum(ir);
			_simil_id_buf.printf(0, "%s",
					contructSimilaritiesID(vec_icl[vec_sort[i]], pos, ref_ind));
			v = similarities.get((const void *) _simil_id_buf.ptr(),
					_simil_id_buf.length());
			if (v) {
				_simil_id_buf.printf(0, "%s",
						contructSimilaritiesID(vec_icl[i], pos, ref_ind));
				*values.set((const void *) _simil_id_buf.ptr(),
						_simil_id_buf.length()) = *v;
			}
		}
	}
	idx * t = 0;
	for (idx i = 0; i < vec_sort.dim(); ++i) {
		for (idx ir = 0; ir < simil_size; ++ir) {
			...
			_simil_id_buf.printf(0, "%s",
					contructSimilaritiesID(vec_icl[i], pos, ref_ind));
			v = values.get((const void *) _simil_id_buf.ptr(),
					_simil_id_buf.length());
			t = similarities.get((const void *) _simil_id_buf.ptr(),
					_simil_id_buf.length());

			if (t) {
				if (v) {
					*similarities.set((const void *) _simil_id_buf.ptr(),
							_simil_id_buf.length()) = *v;
				} else {
					*similarities.set((const void *) _simil_id_buf.ptr(),
							_simil_id_buf.length()) = 0;
				}
			} else if (v) {
				*similarities.set((const void *) _simil_id_buf.ptr(),
						_simil_id_buf.length()) = *v;
...
idx sFrameConsensus::getCloneSupport(idx ic) {
	...
	for (idx i = cl->summary.start; i < cl->summary.start + cl->seqCov.dim();
			++i) {
		clP = cl->getSubPos(i);
#ifdef _DEBUG
		if(!clP) {
...
bool sFrameConsensus::areDifferent(sCloneConsensus * cl1, sCloneConsensus * cl2,
		idx start, idx end) {
	if (start <= 0) {
		start = (cl1->summary.start < cl2->summary.start) ?
				cl2->summary.start : cl1->summary.start;
	}
	if (end <= 0) {
...
idx sFrameConsensus::cleanOutClones(sVec<idx> &cloneIndex) {
	...
	cloneIndex.add(dim());
	cloneIndex.set(0);
	sCloneConsensus * addedclone = 0, *cl = 0, *t_cl = 0, *m_cl = 0, *cl2 = 0,
			*t_cl2 = 0, *m_cl2 = 0;
	...
	for (idx i = 0; i < dim(); ++i) {
		...
		for (idx j = 0; j < dim(); ++j) {
			...
			if (!cl2->isValid())
				continue;
			if (cl->summary.isLink() && cl2->summary.isLink()) {
				if (!(cl->summary.parentClID == cl2->summary.parentClID
						&& cl->summary.mergeclID == cl2->summary.mergeclID))
					continue;
			} else if (cl->summary.isLink() || cl2->summary.isLink())
				continue;
			...
			if (!m_cl2)
				...
				t_cl2 = m_cl2;
			idx start2 = cl2->summary.first_bif_pos, end2 =
					cl2->getSummaryEnd();
			;
			...
			if (!areDifferent(cl, cl2)) {
				...
				if (!cl2->summary.isLink()) {
					if (mergePositions(cl, cl2, 0, 0, true)) {
						updateDependencies(cl2, cl);
						cl2->summary.clID = -1;
					}
				} else {
					if (mergePositions(cl, cl2, cl2->summary.start + 3, 0,
							true)) {
						updateDependencies(cl2, cl);
					}
				}
	...
	for (idx i = 0; i < dim(); ++i) {
		...
		if (t_end > cl->summary.start
				&& t_end <= cl->summary.start + cl->seqCov.dim()) {
			if (t_end - cl->summary.start < 2)
				t_end = cl->summary.start + 2;
		...
		if (cl->summary.hasParent()) {
			...
			if (cl->summary.first_bif_pos < cl->summary.start
					|| cl->summary.first_bif_pos > cl->getSummaryEnd()) {
		...
		for (idx j = offstart; j < addedclone->seqCov.dim(); ++j) {
			remapSimilarities(&cleanClones.similarities, i,
					addedclone->summary.start + j, icl);
	...
	for (idx i = 0; i < cleanClones.similarities.dim(); ++i) {
		...
		*similarities.set(p_id, pLen) = *(idx *) cleanClones.similarities.get(
				p_id, pLen);
	}
	...
	for (idx i = 0; i < cleanClones.dim(); ++i) {
		...
		if (cl->summary.mergeclID >= 0
				&& cl->summary.mergeclID < cloneIndex.dim()) {
			cl->summary.mergeclID = cloneIndex[cl->summary.mergeclID];
		}
		if (cl->summary.parentClID >= 0
				&& cl->summary.parentClID < cloneIndex.dim()) {
...
void sFrameConsensus::getCloneStats(sBioseqpopul::cloneStats * clSum) {
	...
	for (idx i = 0; i < dim(); ++i) {
		...
		for (idx j = 0; j < cl->seqCov.dim(); ++j) {
			cl->stats.avCov += (real) cl->seqCov[j].coverage()
					/ cl->seqCov.dim();
...
bool sFrameConsensus::setConsensusStat(sBioseqpopul::cloneStats &clSum) {
	...
	for (idx i = 0; i < dim(); ++i) {
		...
		if (cl->summary.hasParent())
			parentCl = ptr(cl->summary.parentClID);
		if (cl->summary.hasMerged())
			mergeCl = ptr(cl->summary.mergeclID);
		tSC = &cl->seqCov;
		...
		while (parentCl) {
			tSC = &parentCl->seqCov;
			sq_size = tSC->dim();
			if (sq_size > cl->summary.start - parentCl->summary.start)
				sq_size = cl->summary.start - parentCl->summary.start;
			else
				break;
