...
idx sViopop::Digest(const char* outputfilename, sFrameConsensus * Clones,
		idx * arrSortClon, sBioseqpopul::cloneStats * sumStats,
		idx * sortedSubjInd) {
	...
	idx relationlistClones[8] = { eTypeClone, eTypeClone, eTypeClone,
			eTypeSeqCov, eTypeStats, eTypeSummary, eTypeSortArr };
	...
	sVec<sVec<idx> > clonesSeqCov;
	clonesSeqCov.add(Clones->dim());
	for (idx iCl = 0; iCl < Clones->dim(); ++iCl) {
		sCloneConsensus * clone = Clones->ptr(iCl);

		db.AddRecord(eTypeClone, (const void *) &(clone->summary),
				sizeof(sBioseqpopul::cloneSummary));

		if (clone->summary.clID >= 0) {

			if (clone->summary.parentClID >= 0
					&& clone->summary.clID != clone->summary.parentClID)
				db.AddRecordRelationshipCounter(eTypeClone, 0,
						eClone2FatherClone);

			if (clone->summary.mergeclID >= 0
					&& clone->summary.clID != clone->summary.mergeclID)
				db.AddRecordRelationshipCounter(eTypeClone, 0,
						eClone2MergeClone);
			...
			if (clone->seqCov.dim()) {
				...
				if (db.SetRecordIndexByBody((const void *) seqCov->ptr(),
						eTypeSeqCov, &myRecordID,
						sizeof(idx) * seqCov->dim())) {
					db.AddRecord(eTypeSeqCov, (const void *) (seqCov->ptr()),
							sizeof(idx) * seqCov->dim());
				}
				db.AddRecordRelationshipCounter(eTypeSeqCov, myRecordID,
						eSeqCov2Clone);

				myRecordID = 0;
				if (db.SetRecordIndexByBody((const void *) &(clone->stats),
						eTypeStats, &myRecordID,
						sizeof(sBioseqpopul::cloneStats)))
					db.AddRecord(eTypeStats, (const void *) &(clone->stats),
							sizeof(sBioseqpopul::cloneStats));
			}
	...
	for (idx fCl = 0; fCl < Clones->dim(); ++fCl) {
		...
		if (fclone->summary.clID < 0)
			continue;
		for (idx bCl = 0; bCl < Clones->dim(); ++bCl) {
			if (bCl == fCl)
				continue;
			sCloneConsensus * bclone = Clones->ptr(bCl);
			if (bclone->summary.parentClID >= 0
					&& bclone->summary.parentClID == fclone->summary.clID) {
				db.AddRecordRelationshipCounter(eTypeClone, fCl + 1,
						eClone2Clone);
			}
		}
	}

	db.AddRecord(eTypeSummary, (const void *) sumStats,
			sizeof(sBioseqpopul::cloneStats));
	db.AddRecordRelationshipCounter(eTypeClone, 1, eClone2Summary);
	db.AddRecord(eTypeSortArr, (const void *) arrSortClon,
			sizeof(idx) * Clones->dim());
	...
	for (idx iCl = 0; iCl < Clones->dim(); ++iCl) {
		...
		if (clone->summary.clID < 0)
			continue;
		idx mySeqCov = db.GetRecordIndexByBody(
				(const void *) clonesSeqCov[iCl].ptr(), eTypeSeqCov,
				sizeof(idx) * clonesSeqCov[iCl].dim());
		idx myStats = db.GetRecordIndexByBody((const void *) &(clone->stats),
				eTypeStats, sizeof(sBioseqpopul::cloneStats));
		...
		for (idx bCl = 0; bCl < Clones->dim(); ++bCl) {
			...
			if (bclone->summary.clID == clone->summary.parentClID)
				db.AddRelation(eTypeClone, eClone2FatherClone, iCl + 1,
						bCl + 1);
		}
	}
...
bool sViopop::buildMergeDictionary(ParamCloneIterator * params,
		sDic<sVec<idx> > &mergeTree, idx clCnt) {
	if (!(params->minCov || params->minLen || params->minSup
			|| params->hiddenClones)) {
		return false;
	}
	sStr dst;
	sDic<idx> cl2hide;
	sString::searchAndReplaceSymbols(&dst, params->hiddenClones, 0, ",", 0, 0,
	...
	for (idx iCl = 0; iCl < clCnt; ++iCl) {
		...
		if (!mergeCl
				&& ((params->minCov && clStat->maxCov < params->minCov)
						|| (params->minLen && length < params->minLen)
						|| (params->minSup && getCl(iCl)->hasParent()
								&& clStat->support < params->minSup))) {
			mergeCl = true;
		}
		if (!mergeCl)
...
idx sViopop::buildMergeCompositionDictionary(ParamCloneIterator * params,
		sDic<sVec<contigComp> > &mergeComp) {
...
idx sViopop::iterateClones(idx * iVis, idx start, idx cnt,
		typeCallbackIteratorFunction callbackFunc, ParamCloneIterator * params,
		idx * arrSortClon) {
...
//    idx myVis=0;
	...
	if (params->resolution) {
		...
		for (idx iCl = 0; iCl < clCnt; ++iCl) {
			...
			if (mergeDict) {
				if (isValidClone(clSum->clID)) {
					...
					cComp.cut(0);
					pG2SMap = &g2sMap;
					if (mergeClone(clSum, mergedCl, cComp, params, &seqCov,
							pG2SMap)) {
						clSum = &mergedCl;
					}
				} else
		...
		params->resolution =
				(params->resolution && params->resolution > 0) ?
						(((maxEnd - minStart - 2) / params->resolution > 0) ?
								(maxEnd - minStart - 2) / params->resolution : 1) :
						0;
	}

	if (params->isNormCov) {
		sBioseqpopul::cloneStats * gStats = getGenStats();
		sumPosCov.add(gStats->size);
		sumPosCov.set(0);
		for (idx icl = 0; icl < clCnt; ++icl) {
	...
	for (idx icl = 0; icl < clCnt; ++icl) {
		...
		if (clSum && clSum->clID < 0)
			continue;
		sBioseqpopul::cloneStats * clStat = getStats(iCl);
		if (!clStat)
			continue;
		if (clSum->start == clSum->end)
			continue;
		...
		if (progress_CallbackFunction) {
			if (!progress_CallbackFunction(progress_CallbackParam, iCl,
					100 * (iCl) / clCnt, 100))
				return 0;
		}
	}
...
idx sViopop::clonesCoverage(ParamCloneIterator * params,
		sBioseqpopul::cloneSummary * cl, sVec<seqCovPosition> * seqCov,
		idx refCnt, sVec<contigComp> * comp, sVec<idx> * skp2gapMap,
		sVec<idx> * gap2skpMap) {
	real maxCov = -1;
	...
	params->out->printf("%" DEC ",%" DEC ",", spcFrmRangeStart + 1,
			spcFrmRangeEnd);        //showStart, showEnd
	...
	params->out->printf("Clone_%" DEC ",Clone_%" DEC ",%" DEC ",%" DEC ",",
			parentID, mergeID, bifurcPos, mergePos);
	...
	idx contigMultStart = getSkip2GapContigPos(cl, contigSpcFrmStart,
			skp2gapMap);
	idx contigSpcFrmEnd = spcFrmRangeEnd - spcFrmStart;
	idx contigMultEnd = getSkip2GapContigPos(cl, contigSpcFrmEnd - 1,
			skp2gapMap) + 1;
	...
	if (simil) {
		oSimil.cut(0);
		printSimilaritiesinJSON(oSimil, cl->clID, fi, cov, refCnt, params,
				comp);
	}
	printPositionCoverageinJSON(oCov, ti + 1, c, cov, oSimil.ptr(),
			params->normCov, params->normCov ? params->normCov[fi] : 0);
	maxCov =
			params->normCov ?
					(params->normCov[fi] ?
							100 * (real) cov / params->normCov[fi] : 0) :
					cov;

	idx elms = 0, minPosi = 0, maxPosi = 0;
	real maxIntCov = -REAL_MAX, minIntCov = REAL_MAX;
	sStr tmpOut, minOut, maxOut;
	for (fi = frmMultStart + 1, ci = contigMultStart + 1;
			fi < frmMultEnd - 1 && ci < contigMultEnd - 1; ++fi, ++ci) {

		cPos = getContigPos(cl, ci, gap2skpMap);
		if (cPos < 0
				|| (gap2skpMap && !sCP[ci].base() && !sCP[ci].coverage())) {
			continue;
		}
		ti = spcFrmStart + cPos;
		...
		tmpOut.printf(0, ",");
		printPositionCoverageinJSON(tmpOut, ti + 1, c, cov, oSimil.ptr(),
				params->normCov, params->normCov ? params->normCov[fi] : 0);
	...
	if (cPos >= 0) {
		...
		printPositionCoverageinJSON(oCov, ti + 1, c, cov, oSimil.ptr(),
				params->normCov,
				params->normCov ? params->normCov[cordEnd] : 0);
	}
...
bool sViopop::printSimilaritiesinJSON(sStr & out, idx cl, idx pos, idx cov,
		idx rfCnt, ParamCloneIterator * params, sVec<contigComp> * comp) {
	...
	if (cov) {
		...
		if (sort_sim.dim()) {
			sVec<idx> ofs(sMex::fExactSize);
			ofs.add(sort_sim.dim());
...
void sViopop::printPositionCoverageinJSON(sStr & out, idx pos, char let,
		idx cov, const char * sim, bool norm, idx t_cov) {
...
bool sViopop::mergeClone(sBioseqpopul::cloneSummary * cl,
		sBioseqpopul::cloneSummary &resCl, sVec<contigComp> & comp,
		ParamCloneIterator * params, sVec<seqCovPosition> * seqCov,
		sVec<idx> * l_gap2skpMap, sVec<idx> *l_skp2gapMap) {
	...
	if (!res) {
		...
		idx start = 0, end = 0, sq_size = 0, pos = 0, base = 0, skpPos = 0,
				gapPos = 0;
			...
			if (seqCov) {
				...
				for (idx j = 0; j < ctg->contigInds.dim(); ++j) {
					...
					for (idx l = start; l < end; ++l, ++pos) {
						...
						if (c_seqCov[l].isMultiGap()) {
							(profpos->pos[sBioseqpopul::baseGap]) +=
									c_seqCov[l].coverage() ?
											c_seqCov[l].coverage() : 1;
						} else {
							profpos->pos[c_seqCov[l].base()] +=
									c_seqCov[l].coverage();
						}
					}
				}
				for (idx j = 0; j < prof.dim(); ++j) {
					...
					if (prof[j].isMultiGap()) {
						r_seqCov->setCoverage(
								prof[j].pos[sBioseqpopul::baseGap]);
						r_seqCov->setBase(sBioseqpopul::baseGap);
						if (l_gap2skpMap && gap2skipMap)
							*(l_gap2skpMap->add()) = -1;
						++gapPos;
					} else if (prof[j].isDeletion()
							&& (gapFlag & clSkipSupportedDeletions)) {
						r_seqCov->setCoverage(
								prof[j].pos[sBioseqpopul::baseDel]);
						...
						if (l_gap2skpMap && gap2skipMap)
							*(l_gap2skpMap->add()) = skpPos++;
						if (l_skp2gapMap && skip2gapMap)
							*(l_skp2gapMap->add()) = gapPos;
						++gapPos;
					}
				}
			}
	...
	if (cl && mergeDict) {
		...
		if (merged) {
			...
			for (idx j = 0; j < comp->dim(); ++j) {
				if (cl->start <= comp->ptr(j)->start
						&& cl->end >= comp->ptr(j)->end) {
			...
			for (idx i = 0; i < merged->dim(); ++i) {
				...
				for (idx j = 0; j < comp->dim() && m_cl; ++j) {
					if (m_cl->start <= comp->ptr(j)->start
							&& m_cl->end >= comp->ptr(j)->end) {
...
bool sViopop::getValidCloneComposition(sBioseqpopul::cloneSummary * cl,
		sVec<contigComp> & c_comp) {
	c_comp.cut(0);
	if (mergeCompDict) {
...
idx sViopop::printAllClones(sViopop * viopop, ParamCloneIterator * params,
		idx clIndex) {
	...
	if (viopop->mergeDict) {
		if (viopop->isValidClone(cl->clID)) {
			if (viopop->mergeClone(cl, mergedCl, cComp, params, &seqCov,
					pG2Smap, pS2Gmap)) {
				pSC = &seqCov;
				cl = &mergedCl;
				if (cComp.dim()) {
	...
	params->out->printf("\n");
	params->out->printf("Clone_%" DEC ",%" DEC ",%" DEC ",", cl->clID,
			viopop->getFramePos(cl, cl->start, pG2Smap, true) + 1,
			viopop->getFramePos(cl, cl->end - 1, pG2Smap, true) + 1);

	if (viopop->clonesCoverage(params, cl, pSC, refCnt, pC_Comp, pS2Gmap,
			pG2Smap) < 0)
		return 0;

	params->out->printf(",");
	sBioseqpopul::cloneSummary * pCl = viopop->getValidCloneSummary(
			cl->parentClID);
	if (cl->hasParent() && pCl) {
		...
		viopop->clonesDifferences(params, pCl, cl, ppSC, pSC, pG2Smap,
				viopop->getStats(cl->clID));
	} else {
		params->out->printf("-1,,,,,");
	}
...
idx sViopop::clonesDifferences(ParamCloneIterator * params,
		sBioseqpopul::cloneSummary * parentCl,
		sBioseqpopul::cloneSummary * childCl, sVec<seqCovPosition> * pSC,
		sVec<seqCovPosition> * cSC, sVec<idx> * child_gap2skpMap,
		sBioseqpopul::cloneStats * c_stats) {
	...
	idx start =
			(childCl->start < parentCl->start) ?
					parentCl->start : childCl->start;
	...
	for (idx i = start; i < end; ++i) {
		...
		if (isDiff) {
			...
			oDifs.printf("%" DEC ">%" DEC "/%c:%" DEC "/%c", i + 1,
					pSeqCov[pi].coverage(), fB, cSeqCov[ci].coverage(), cB);
			lastDif = i + 1;
			++diffs;
		}
	}
	oDifs.printf("\"");
	params->out->printf("%" DEC ",%" DEC ",%" DEC ",%s",
			getFramePos(childCl, firstDif, child_gap2skpMap, true),
			getFramePos(childCl, lastDif, child_gap2skpMap, true), diffs,
			oDifs.ptr());

	if (c_stats)
		params->out->printf(",\"{\'pvalue\':%lf,\'bayes\':%lf}\"",
				c_stats->bifurcation_pvalue, c_stats->bifurcation_bayes);

	return diffs;
}

idx sViopop::printHierarchySingle(sViopop * viopop, ParamCloneIterator * params,
		idx clIndex) {
	...
	if (clSum && clSum->clID >= 0 && clSum->start != clSum->end) {
		params->out->printf("Clone_%" DEC ",%" DEC ",%" DEC ",%" DEC,
				clSum->clID, clSum->start, clSum->end, clSum->mergeclID);
...
idx sViopop::printAllCoverageClones(idx * iVis, idx start, idx cnt, sStr & out,
		idx * arrSortClon) {
	idx * clList = 0;
...
//static
idx sViopop::getClonesOnPosition(sViopop * viopop, idx pos,
		sVec<idx> * clInds) {
...
idx sViopop::getBifurcatedClonesOnPosition(idx pos, sVec<idx> * clInds,
		idx clId) {
	...
	for (idx cLi = 0; cLi < dimCl(); ++cLi) {
		...
		if (merged
				&& !merged->get((const void *) _t_buf.ptr(), _t_buf.length())) {
			continue;
		}
		clSum = getCl(cLi);
		if (clSum && pos == clSum->start
				&& getValidCloneID(clSum->parentClID) == clId) {
			*clInds->add() = cLi;
		}
	}
	return clInds->dim();
...
bool sViopop::isCloneAncestor(sBioseqpopul::cloneSummary * clA,
		sBioseqpopul::cloneSummary * clB) {
	sBioseqpopul::cloneSummary * parentCl = 0;
	if (clB->hasParent())
		parentCl = getCl(clB->parentClID);
	while (parentCl) {
...
bool sViopop::isCloneAncestor(sBioseqpopul::cloneSummary * t_sum,
		sDic<bool> & clones) {
...
bool sViopop::cloneOverlap(sBioseqpopul::cloneSummary * clA,
		sBioseqpopul::cloneSummary * clB) {
	return sOverlap(clA->start, clA->end, clB->start, clB->end);
}

bool sViopop::cloneOverlap(sBioseqpopul::cloneSummary * clSum,
		sDic<bool> & clones) {
	sBioseqpopul::cloneSummary * t_cl = 0;
	for (idx i = 0; i < clones.dim(); ++i) {
		t_cl = getCl(*(idx *) clones.id(i));
...
idx sViopop::getPermutationsCompositions(const char * clones_input,
		sVec<sVec<sViopop::cloneRegion> > & out, sVec<idx> & vclIds,
		ParamCloneIterator * params) {
	sDic<bool> clIds;
	sVec<idx> vec_clIds;
	...
	for (idx c = 0; c < vec_clIds.dim(); ++c) {
		if (mergeDict
				&& !mergeDict->get(t_buf.printf(0, "%" DEC, vec_clIds[c])))
			continue;
		else
	...
	while (clonesParsed.dim() && cnt < params->cnt) {
		...
		for (pos = lastcl.start; pos < gen->size; ++pos) {
			if (!found && pos
					&& getBifurcatedClonesOnPosition(pos, &vec_clIds,
							cl->clID)) {
				for (idx j = 0; j < vec_clIds.dim(); ++j) {
					...
					if (clIds.find((void *) &t_cl->clID, sizeof(idx))
							|| isCloneAncestor(t_cl, clIds)) {
						cl = t_cl;

						c_Out->ptr(c_Out->dim() - 1)->end =
								pos
										- getCl(
												c_Out->ptr(c_Out->dim() - 1)->contigInd)->start;
						t_Contig = c_Out->add();
					...
					if (!cloneOverlap(t_cl, clIds)) {
						...
						*clonesParsed.push() = lastcl;
						...
						t_Out->copy(c_Out);
						if (t_Out->dim()) {
							t_Out->ptr(t_Out->dim() - 1)->end =
									pos
											- getCl(
													t_Out->ptr(t_Out->dim() - 1)->contigInd)->start;
						}
						t_Contig = t_Out->add();
						t_Contig->contigInd = lastcl.clID;
						t_Contig->start = 0;
...
idx sViopop::getExtendedComposition(sBioseqpopul::cloneSummary * cl,
		sVec<sViopop::cloneRegion> * c_Out, ParamCloneIterator * params) {
	if (!c_Out || !cl) {
		return 0;
	}

	sVec<contigComp> comp;
	sBioseqpopul::cloneSummary rsCl, *parentCl = 0, rsParnentCl, *mergeCl = 0,
			rsMergedCl, *orCl = 0;
	sStr title;
...
idx sViopop::getExtendedCompositions(const char * clones_input,
		sVec<sVec<sViopop::cloneRegion> > & out, sVec<idx> & clIds,
		ParamCloneIterator * params) {
	sVec<idx> vec_clIds;
...
idx sViopop::getContigCompositions(const char * clones_input,
		sVec<sVec<sViopop::cloneRegion> > & out, sVec<idx> & clIds,
		ParamCloneIterator * params) {
	sVec<idx> vec_clIds;
	sBioseqpopul::cloneSummary * cl;
...
idx sViopop::getContigComposition(sBioseqpopul::cloneSummary * cl,
		sVec<sViopop::cloneRegion> * c_Out, ParamCloneIterator * params) {
	sVec<idx> vec_clIds;
...
struct rangeGraphNode {
	...
	bool isForwardConnection(sBioseqpopul::cloneSummary * current,
			sBioseqpopul::cloneSummary * next) {
		return (next->parentClID == current->clID && next->start == start)
				|| (next->clID == current->mergeclID && current->end == start);
	}
	bool isBackwardConnection(sBioseqpopul::cloneSummary * current,
			sBioseqpopul::cloneSummary * previous) {
		return (previous->clID == current->parentClID && current->start == end)
				|| (previous->mergeclID == current->clID && previous->end == end);
	}
	bool isForwardRelated(sBioseqpopul::cloneSummary * current,
			sBioseqpopul::cloneSummary * next) {
		return (next->parentClID == current->clID)
				|| (next->clID == current->mergeclID);
	}
	bool isBackwardRelated(sBioseqpopul::cloneSummary * current,
			sBioseqpopul::cloneSummary * previous) {
		return (previous->clID == current->parentClID)
				|| (previous->mergeclID == current->clID);
	}
	...
	idx getClosestContig(sViopop * vp, sBioseqpopul::cloneSummary * cc, real fr,
			bool forward, real minF) {
		sBioseqpopul::cloneSummary * ic = 0;
		real minFrDistance = REAL_MAX;
		idx iR = -1;
		bool unconnected = true, unrelated = true;
		for (idx i = 0; i < cls.dim(); ++i) {
			ic = &cls[i].iCl;
			if ((cls[i].iCl.clID == cc->clID)
					|| (forward && isForwardConnection(cc, ic))
					|| (!forward && isBackwardConnection(cc, ic))) {
				unconnected = false;
				if (cls[i].fr > minF) {
		...
		if (iR < 0 && !unconnected) {
			for (idx i = 0; i < cls.dim(); ++i) {
				ic = &cls[i].iCl;
				if ((cls[i].iCl.clID == cc->clID)
						|| (forward && isForwardRelated(cc, ic))
						|| (!forward && isBackwardRelated(cc, ic))) {
					unrelated = false;
					if (cls[i].fr > minF) {
						if (sAbs(fr - cls[i].fr) < minFrDistance) {
							minFrDistance = sAbs(fr - cls[i].fr);
...
real getPath(sViopop * vp, sVec<rangeGraphNode> & grph, idx iNode,
		sVec<sViopop::cloneRegion> & out, real t_minF) {
	...
	for (idx i = iNode - 1; i >= 0; i--) {
		iClGr[i] = grph[i].getClosestContig(vp,
				&grph[i + 1].cls[iClGr[i + 1]].iCl, iF, (iNode < i), t_minF);
		if (iClGr[i] < 0) {
			start_node = i;
			break;
		}
...
void normalizeRangeGraph(sVec<rangeGraphNode> &rangeGraph) {
		...
		for (idx p_j = 0; p_j < prv_gn->cls.dim(); ++p_j) {
			...
			if (prv_gn->cls[p_j].normalized_len == 0) {
				if (!prv_gn->isConnected(p_j, gn)) {
					...
					gn->cls[j].fr = ((gn->cls[j].fr * gn->length())
							+ (prv_gn->cls[p_j].fr * norm_len))
							/ (gn->length() + norm_len);
	...
	for (idx i = 0; i < rangeGraph.dim(); ++i) {
		...
		real coef =
				(tot_sum != const_sum) ?
						(1 - const_sum) / (tot_sum - const_sum) : 1;
...
idx sViopop::getPredictedGlobal(sVec<sVec<sViopop::cloneRegion> > & out,
		sVec<real> & frequencies, ParamCloneIterator * params) {
...
idx popGraph::buildGraph(sViopop::ParamCloneIterator * params) {
	sVec<idx> rng_cl(sMex::fSetZero | sMex::fExactSize), sorted_cl_rngs(
			sMex::fSetZero);
	...
	for (idx i = 1; i < sorted_cl_rngs.dim(); ++i) {
		...
		for (idx j = 0; j < res.dim(); ++j) {
			if (!_vp->mergeClone(_vp->getCl(res[j]), cl_sums[j], comp, 0,
					seqCovs.ptr(j))) {
				seqCovs.ptr(j)->cut(0);
		...
		for (idx j = 0; j < res.dim(); ++j) {
			...
			if (cl_sum.start == c_start) {
				...
				if (cn->clone.hasParent()) {
					idx i_pnode = getNodeIndFromRange(cn->clone.parentClID,
							c_start);
				...
				for (idx k = 0; k < cl_sums.dim(); ++k) {
					if (cl_sums[k].start == c_start && cl_sums[k].hasParent()
							&& cl_sums[k].parentClID == cl_sum.clID)
						++bifurc_cnt;
				}
			...
			real cr_cov = 0;
			for (idx ic = c_start - cn->clone.start;
					ic < c_end - cn->clone.start; ++ic) {
...
idx popGraph::traverse(idx seed_ind, sVec<sViopop::cloneRegion> & out,
		bool min_diversity) {
...
idx sViopop::printContigsPrevalence(
		sVec<sVec<sViopop::cloneRegion> > & compositions, sVec<idx> & clIds,
		sStr & out, ParamCloneIterator * param) {
	if (compositions.dim() <= 0)
		return 0;
	...
	for (idx ic = 0; ic < compositions.dim(); ++ic) {
		...
		for (idx i = 0; i < composition.dim(); ++i) {
			...
			printTitle(title, param, t_region->contigInd, offset + 1,
					end - start + offset, "|");

			for (idx pos = t_region->start; pos < t_region->end;
					++pos, ++offset) {
				if (getContigPos(cl, pos, pG2S) < 0) {
					--offset;
					continue;
				}
				if (isAnyGap(cl, pos, pG2S) || tSC[pos].base() == 4
						|| (param->covThrs
								&& param->covThrs >= tSC[pos].coverage())) {
					if (!isSkpGaps) {
						len++;
					} else {
						--offset;
		...
		if (avCov) {
			...
			totAvCovs += avCov;
			if (param->fastaTmplt) {
				sString::searchAndReplaceStrings(&titles, param->fastaTmplt, 0,
						"$_(v)" __, title.ptr(1), 1, false);
				titles.shrink00();
			} else {
	...
	idx validCovs = 0;
	idx cnt =
			param->userPointer ? (*(idx*) (param->userPointer)) : avCovs.dim();
	if (cnt > avCovs.dim())
		cnt = avCovs.dim();
	for (idx ic = 0; ic < cnt; ++ic) {
		if (avCovs[ic]) {
			++validCovs;
			out.printf("%s,%.4f,%.4f\n", sString::next00(titles, ic),
					avCovs[ic],
					param->frequencies ?
							param->frequencies[ic] :
							(100 * avCovs[ic] / totAvCovs));
		}
	}
	return validCovs;
}

idx sViopop::printContig(sVec<sVec<sViopop::cloneRegion> > & composition,
		sVec<idx> & clIds, sStr & out, ParamCloneIterator * params,
		idx print_type) {
	idx res = 0;
	switch (print_type) {
	case ePrintContigSeq:
		for (idx i = 0; i < composition.dim(); ++i) {
			res = printContigSequences(composition[i], out, params, clIds[i],
					0);
		}
		break;
	case ePrintContigAl:
		for (idx i = 0; i < composition.dim(); ++i) {
			res = printContigAlignments(composition[i], out, params, clIds[i],
					0);
		}
		break;
	case ePrintContigCov:
		for (idx i = 0; i < composition.dim(); ++i) {
			res = printContigCoverages(composition[i], out, params, clIds[i],
					0);
		}
		break;
	case ePrintContigComp:
		for (idx i = 0; i < composition.dim(); ++i) {
			res = printContigComposition(composition[i], out, params, clIds[i],
					0);
		}
		break;
	case ePrintContigBreakpoints:
		for (idx i = 0; i < composition.dim(); ++i) {
			res = printContigBreakpoints(composition[i], out, params, clIds[i],
					0);
		}
		break;
	case ePrintContigSummary:
		params->out->printf(
				"Clone ID,Start,End,Merged ID,Bifurcated ID,Bifurcation start,Bifurcation End,Supporting positions,Differences\n");
		for (idx i = 0; i < composition.dim(); ++i) {
...
bool appendToLastComp(sStr & title, idx &contgId, idx &start, idx &end,
		const char * sep) {
	char * src = title.ptr();
	bool isAppended = false;
	char * last = sString::searchSubstring(src, title.length(), "contig_" __,
			sNotIdx, "", false);
	if (last) {
		idx prevCntId, prevStart, prevEnd;
		idx ret = sString::bufscanf(last, title.last(),
				"contig_%" DEC "[%" DEC "-%" DEC "]", &prevCntId, &prevStart,
				&prevEnd);
		if (ret == 3 && prevCntId == contgId) {
			title.printf((idx) (last - src),
					"contig_%" DEC "[%" DEC "-%" DEC "]", contgId, prevStart,
					end);
			isAppended = true;
		}
	}
...
const char * sViopop::printTitle(sStr & title, ParamCloneIterator * param,
		idx contgId, idx start, idx end, const char * sep) {
	if (!sep)
		sep = ",";
	...
	} else if (param && (param->flags & clPrintFastaTitleNumbersOnly)
			&& !title.length()) {
...
idx sViopop::printContigSequences(sVec<sViopop::cloneRegion> & composition,
		sStr & out, ParamCloneIterator * param, idx cur_clId, idx offset) {
	if (composition.dim() <= 0)
		return 0;
	...
	for (idx i = 0; i < composition.dim(); ++i) {
		...
		for (idx pos = t_region->start; pos < t_region->end; ++pos, ++offset) {
			...
			t_b = tSC[pos].base();
			if (isAnyGap(cl, pos, pG2S) || t_b == 4
					|| (param->covThrs && param->covThrs >= tSC[pos].coverage())) {
				if (!isSkpGaps) {
					seq.printf("-");
				} else {
					--offset;
	...
	if (param->fastaTmplt) {
		sString::searchAndReplaceStrings(&out, param->fastaTmplt, 0, "$_(v)" __,
				title.ptr(1), 1, false);
		out.shrink00();
	} else {
...
idx sViopop::printContigAlignments(sVec<sViopop::cloneRegion> & composition,
		sStr & out, ParamCloneIterator * param, idx cur_clId, idx offset) {
	if (composition.dim() <= 0)
		return 0;
	sViopop::cloneRegion * t_region;
...
idx sViopop::printContigCoverages(sVec<sViopop::cloneRegion> & composition,
		sStr & out, ParamCloneIterator * param, idx cur_clId, idx offset) {
	if (composition.dim() <= 0)
		return 0;
	sViopop::cloneRegion * t_region;
	...
	for (idx i = 0; i < composition.dim(); ++i) {
		...
		for (pos = t_region->start; pos < t_region->end; ++pos, ++offset) {
			...
			if (isAnyGap(cl, pos) || t_b == 4
				...
				seq.printf(",%" DEC,
						(idx) (tSC[pos].coverage() * (t_region->freq)));
			}
		}
		pos += cl->start;
	}
...
idx sViopop::printContigComposition(sVec<sViopop::cloneRegion> & composition,
		sStr & out, ParamCloneIterator * param, idx cur_clId, idx offset) {
	if (composition.dim() <= 0)
		return 0;
	sViopop::cloneRegion * t_region;
	...
	for (idx i = 0; i < composition.dim(); ++i) {
		...
		if (!(param->flags & clPrintNoGapsFrame)
				&& (param->flags & clPrintContigsInMutualFrame)) {
			seq.printf("%" DEC " %" DEC " clone_%" DEC ":%" DEC "-%" DEC "\n",
					offset + 1, end - start + offset, t_region->contigInd,
					cl->start + start + 1, cl->start + end);
		} else {
			seq.printf("%" DEC " %" DEC " clone_%" DEC ":%" DEC "-%" DEC "\n",
					offset + 1, end - start + offset, t_region->contigInd,
					start + 1, end);
...
idx sViopop::printContigBreakpoints(sVec<sViopop::cloneRegion> & composition,
		sStr & out, ParamCloneIterator * param, idx cur_clId, idx offset) {
	if (composition.dim() <= 0)
		return 0;
	sBioseqpopul::cloneStats * gStats = getGenStats();
	...
	for (idx i = 0; i < composition.dim(); ++i) {
		...
		for (idx pos = t_region->start; pos < t_region->end; ++pos, ++offset) {
			...
			getPositionSimil(cl->clID, cl->start + pos, refCnt, similsV.ptr(),
					true, pComp);

			cnt_valid_simil = getSimilAboveThrshld(similsV, simils,
					simil_thrshld, (sBioseq *) param->userPointer);
			if (first) {
				t_simils.printf(0, "%s", simils.ptr());
				first = false;
			}
			if (!cnt_valid_simil
					&& !(param->flags & sViopop::clPrintLowDiversityBreaks)) {
				continue;
			}
			if (isBreakpoint(t_simils, simils)) {
				seq.printf(" %" DEC " %s\n%" DEC, offset, t_simils.ptr(),
						offset + 1);
				t_simils.printf(0, "%s", simils.ptr());
			}
		}
	...
	printTitleFreq(title, param, cur_clId);
	if (param->fastaTmplt) {
		sString::searchAndReplaceStrings(&t_title, param->fastaTmplt, 0,
				"$_(v)" __, title.ptr(1), 1, false);
		out.shrink00();
	} else {
...
idx sViopop::printContigSummary(sVec<sViopop::cloneRegion> & composition,
		sStr & out, ParamCloneIterator * params, idx offset) {
...
idx sViopop::printAllSequenceClones(idx * iVis, idx start, idx cnt, sStr & out,
		idx * arrSortClon) {
	idx * clList = 0;
...
idx sViopop::printCoverageSingle(sViopop * viopop, ParamCloneIterator * params,
		idx clIndex) {
...
idx sViopop::printStackedStats(sStr * out, ParamCloneIterator * params) {
	...
	for (idx i = stck_lengths.dim() - 1; i >= 0; --i) {
		...
		out->printf("Clone_%" DEC ",%" DEC ",%" DEC "\n", summ_cl->clID,
				stat_cl->stacked_size, stat_cl->size);
	}

	return stck_lengths.dim();
}

idx sViopop::getContigDistance(sBioseqpopul::cloneSummary * cl, idx end,
		idx start, sVec<idx> * contigMap, bool forceValid) {
	if ((!gap2skipMap || !gap2skipMap->dim())
			&& (!contigMap || !contigMap->dim())) {
...
idx sViopop::getGap2SkipFramePos(sBioseqpopul::cloneSummary * cl, idx pos,
		sVec<idx> * contigMap, bool forceValid) {
	idx clPos = 0;

	clPos = getContigPos(cl, pos - cl->start, contigMap, forceValid);
	sBioseqpopul::cloneSummary * parentCl =
			cl->hasParent() ? getValidCloneSummary(cl->parentClID) : 0;
	if (parentCl) {
		sVec<contigComp> comp;
		...
		if (mergeClone(parentCl, resCl, comp, 0, &sc, &t_contigMap)) {
			clPos += sViopop::getGap2SkipFramePos(&resCl, cl->start,
					&t_contigMap, true);
		} else {
		...
		if (m_cl && (!m_cl->hasParent() || m_cl->parentClID != cl->clID)) {
			idx mergePos, contigLen = getContigPos(cl, cl->end - cl->start - 1,
					contigMap, forceValid);
			...
			if (mergeClone(m_cl, resCl, comp, 0, &sc, &t_contigMap)) {
				mergePos = getFramePos(m_cl, cl->end - 1, &t_contigMap, true)
						+ 1;
			} else {
...
idx sViopop::getMergedFramePos(sBioseqpopul::cloneSummary * cl, idx pos,
		ParamCloneIterator * params, bool forceValid) {
...
idx sViopop::getMergedContigPos(sBioseqpopul::cloneSummary * cl, idx pos,
		ParamCloneIterator * params, bool forceValid) {
	sBioseqpopul::cloneSummary mergedCl;
	sVec<seqCovPosition> seqCov;
	sVec<contigComp> cComp;
